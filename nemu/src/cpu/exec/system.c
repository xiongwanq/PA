...
//
//make_EHelper(in) {
//  uint32_t t = pio_read(id_src->val, id_dest->width);
...
make_EHelper(lidt) {
  if(decoding.is_operand_size_16){
    cpu.idtr.limit = vaddr_read(id_dest->addr,2);//读取2字节
    cpu.idtr.base = vaddr_read(id_dest->addr+2,3);//顺序读取3字节
  }
  else{
    cpu.idtr.limit = vaddr_read(id_dest->addr,2);//读取2字节
    cpu.idtr.base = vaddr_read(id_dest->addr+2,4);//顺序往后读取4字节
  }
